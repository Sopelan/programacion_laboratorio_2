...
namespace SP
{
    ...
    public partial class SegundoParcial: Form
    {
        ...
        //Implementar (explicitamente) 7788 en manzana
        ...
        private static string ObtenerListadoFrutas()
        {
            ...
            string cadena = "SELECT * FROM frutas";
            try
            {
                ...
                while (lector.Read())
                {
                    stringBuilder.AppendLine(lector.GetValue(0).ToString() + " " + lector.GetValue(1).ToString() + " " + lector.GetValue(2).ToString() + lector.GetValue(3).ToString());
                }
